<%-- 
    Document   : viewAllCustomPizza
    Created on : Dec 24, 2024, 5:35:31 PM
    Author     : suraj
--%>

<%@ page contentType="text/html" pageEncoding="UTF-8" %>
<%@ page import="java.util.List" %>
<%@ page import="model.CustomPizza" %>
<%@ page import="service.CustomPizzaService" %>

<%
    // Retrieve the list of custom pizzas using the CustomPizzaService
    CustomPizzaService customPizzaService = new CustomPizzaService();
...
<head>
    ...
    <title>Custom Pizza - Pizza Store</title>
...
<body>
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark" id="ftco-navbar">
    ...
    <!-- Main Content -->
    <div class="container">
        <h2><i class="fas fa-sauce mr-3"></i>Custom Pizza</h2>
        ...
        <table class="table">
                            ...
                            <tbody>
                                <% for (CustomPizza pizza : customPizzas) { %>
                                    <tr>
                                        <td><%= pizza.getCustomPizzaID() %></td>
                                        <td><%= pizza.getCustomPizzaName() %></td>
                                        <td><%= pizza.getCrustID() %></td> <!-- You may want to replace with actual crust name from another service -->
                                        <td><%= pizza.getSauceID() %></td> <!-- Same for sauce type -->
                                        <td><%= pizza.getCheeseID() %></td> <!-- Same for cheese type -->
                                        <td><%= pizza.getToppingIDs() %></td> <!-- Show a comma-separated list of toppings -->
                                        <td>$<%= pizza.getTotalPrice() %></td>
                                        <td>
                                            <!-- Action buttons for Edit and Delete -->
                                            <a href="CustomPizzaController?action=delete&customPizzaID=<%= pizza.getCustomPizzaID() %>" 
                                               class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this pizza?')">Delete</a>
                                          
                                            <a href="PizzaController?action=order&pizza_ID=<%= pizza.getCustomPizzaID() %>" class="btn btn-warning">Order</a>
