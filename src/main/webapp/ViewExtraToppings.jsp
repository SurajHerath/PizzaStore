<%-- 
    Document   : ViewExtraToppings
    Created on : Dec 23, 2024, 7:33:37 PM
    Author     : suraj
--%>
...
<body>
    ...
    <!-- Main Content -->
    <div class="container">
        <h2><i class="fas fa-cheese mr-3"></i>List of Extra Toppings</h2>
        <a href="InsertExtraTopping.jsp" class="btn-add"><i class="fas fa-plus mr-2"></i>Add New Extra Topping</a>
...
<c:if test="${not empty toppingList}">
   <table class="table">
        ...
        <tbody>
            <!-- Loop through each extra topping and display its details -->
            <c:forEach var="topping" items="${toppingList}">
                <tr>
                    <td>${topping.extraTopping_ID}</td>
                    <td>${topping.extraTopping_Name}</td>
                    <td>${topping.quantity_Grams}</td>
                    <td>
                        <!-- Edit button - redirects to the edit page -->
                       <a href="ExtraToppingController?action=edit&extraTopping_ID=${topping.extraTopping_ID}"class="btn btn-primary btn-sm">Edit</a>

                        |
                        <!-- Delete button - redirects to the delete action in the controller -->
                        <a href="ExtraToppingController?action=delete&extraTopping_ID=${topping.extraTopping_ID}"class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this topping?')">Delete</a>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
...
<!-- Display message if no toppings are found -->
<c:if test="${empty toppingList}">
    <p>No extra toppings found.</p>
</c:if>
