<%-- 
    Document   : ViewCrust
    Created on : Dec 24, 2024, 4:21:44 PM
    Author     : suraj
--%>
...
<head>
    ...
    <title>All Crust - Pizza Store</title>
...
<body>
    ...
    <!-- Main Content -->
    <div class="container">
        <h2><i class="fas fa-cheese mr-3"></i>All Crust</h2>
        <a href="insertCrust.jsp" class="btn-add"><i class="fas fa-plus mr-2"></i>Add New Crust</a>
        <table class="table">
        ...
        <tbody>
            <c:forEach var="crust" items="${crustList}">
                <tr>
                    <td>${crust.crustID}</td>
                    <td>${crust.crustName}</td>
                    <td>${crust.crustPrice}</td>
                    <td>
                        <a href="CrustController?action=edit&crust_ID=${crust.crustID}"class="btn btn-primary btn-sm">Edit</a> | 
                        <a href="CrustController?action=delete&crust_ID=${crust.crustID}" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this crust?');">Delete</a>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
